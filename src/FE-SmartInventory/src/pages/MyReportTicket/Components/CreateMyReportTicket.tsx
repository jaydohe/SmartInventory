import QRScanner from '@/Components/QRScanner';
import { QueryKeys } from '@/Constant';
import { useBuilderQuery, useUploadFile } from '@/hook';
import { TBuilderQuery } from '@/interface';
import { useQueryReportTicket } from '@/pages/ReportTicket/Hook/useQueryReportTikcet';
import { authStoreSelectors } from '@/Stores/userStore';
import { normFile } from '@/utils';
import {
  CheckOutlined,
  InboxOutlined,
  QrcodeOutlined,
  UnorderedListOutlined,
} from '@ant-design/icons';
import { useQueryClient } from '@tanstack/react-query';
import { Button, Form, Input, Select, Typography, Upload } from 'antd';
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router';
const { Option } = Select;
export interface ICreateMyReportTicketProps {}

export default function CreateMyReportTicket(props: ICreateMyReportTicketProps) {
  const [form] = Form.useForm();
  const [isQrModalVisible, setIsQrModalVisible] = useState(false);
  const [deviceCode, setDeviceCode] = useState<boolean>(false);
  const { uploadFile } = useUploadFile();
  const queryClient = useQueryClient();
  const userId = authStoreSelectors.use.userId() ?? '';
  const navigate = useNavigate();
  const { createTicket } = useQueryReportTicket('');

  const handleSubmit = async (values: any) => {
    const uploadFilePath = values.filePath
      ? uploadFile.mutateAsync(values.filePath)
      : Promise.resolve(null);

    const uploadImagePath = values.imagePath
      ? uploadFile.mutateAsync(values.imagePath)
      : Promise.resolve(null);
    const [filePath, imagePath] = await Promise.all([uploadFilePath, uploadImagePath]);

    if (imagePath) {
      values.imagePath = imagePath.url;
    }
    if (filePath) {
      values.filePath = filePath.url;
    }

    const newData = { ...values };
    createTicket.mutate(newData, {
      onSuccess: (data) => {
        form.resetFields();
        queryClient.invalidateQueries({ queryKey: [QueryKeys.ALL_DEVICE] });
        navigate('/my-report');
      },
    });
  };
  const handleQRScanSuccess = (macAddress: string) => {
    form.setFieldsValue({
      deviceCode: macAddress,
    });
    setDeviceCode(true);
  };
  return (
    <div className="m-4">
      <h2 className="font-bold text-xl md:text-2xl mb-2  text-inherit text-pretty   uppercase text-primary">
        <UnorderedListOutlined className="text-xl mr-2" />
        Báo sự cố của tôi
      </h2>
      <QRScanner
        title="Báo cáo sự cố"
        isVisible={isQrModalVisible}
        onClose={() => setIsQrModalVisible(false)}
        onScanSuccess={handleQRScanSuccess}
      />

      <Form form={form} onFinish={handleSubmit} layout="vertical">
        <Form.Item
          name="deviceCode"
          label={
            <div className="flex items-center gap-2  flex-wrap-reverse">
              Địa chỉ MAC thiết bị
              <Button
                className="py-0 px-4 w-fit bg-infoColor hover:bg-infoColorHover"
                // size="large"
                type="primary"
                icon={<QrcodeOutlined />}
                onClick={() => {
                  setIsQrModalVisible(true);
                }}
              >
                Quét QR mã thiết bị
              </Button>
            </div>
          }
          className="w-full text-base font-semibold "
          rules={[{ required: true, message: 'Vui lòng quét QR mã thiết bị' }]}
        >
          <Input
            readOnly
            disabled={!deviceCode}
            placeholder="Quét QR mã thiết bị"
            className="cursor-not-allowed"
          />
        </Form.Item>
        <Form.Item
          name="name"
          className="w-full text-base font-semibold"
          label="Tên sự cố"
          rules={[{ required: true }]}
        >
          <Input placeholder="Nhập tên sự cố" />
        </Form.Item>
        <Form.Item
          name="description"
          rules={[{ required: true }]}
          className="w-full text-base font-semibold "
          label="Mô tả sự cố"
        >
          <Input.TextArea rows={4} placeholder="Nhập mô tả sự cố" />
        </Form.Item>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
          <Form.Item
            className="w-full text-base font-semibold "
            name="imagePath"
            label="Ảnh thiết bị"
            valuePropName="file"
            getValueFromEvent={normFile}
            rules={[
              { required: true },
              {
                validator: async (_, file) => {
                  if (file && file[0]) {
                    const isLt50M = file[0].size / 1024 / 1024 < 50;
                    if (!isLt50M) {
                      throw new Error('Dung lượng tải lên không được vượt quá 50MB');
                    }
                  }
                  return Promise.resolve();
                },
              },
            ]}
          >
            <Upload.Dragger
              name="File"
              action={`${import.meta.env.VITE_BASE_URL_}/uploads/`}
              multiple
              accept="image/*"
              maxCount={1}
              listType="picture"
              beforeUpload={(file) => {
                // console.log({ file });
                return false;
              }}
            >
              <p className="ant-upload-drag-icon mb-0">
                <InboxOutlined />
              </p>
              <p className="ant-upload-text">
                {'Nhấp hoặc kéo tệp vào khu vực này để tải ảnh lên '}
              </p>
              <p className="ant-upload-hint">
                {'File tải lên có định dạng là: JPG/PNG | Tối đa 50MB'}
              </p>
            </Upload.Dragger>
          </Form.Item>
          <Form.Item
            className="w-full text-base font-semibold "
            name="filePath"
            label="Tệp đính kèm"
            valuePropName="file"
            getValueFromEvent={normFile}
            rules={[
              {
                validator: async (_, file) => {
                  if (file && file[0]) {
                    const isLt50M = file[0].size / 1024 / 1024 < 50;
                    if (!isLt50M) {
                      throw new Error('Dung lượng tải lên không được vượt quá 50MB');
                    }
                  }
                  return Promise.resolve();
                },
              },
            ]}
          >
            <Upload.Dragger
              name="File"
              action={`${import.meta.env.VITE_BASE_URL_}/uploads/`}
              multiple
              accept=".pdf,.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
              maxCount={1}
              listType="picture"
              beforeUpload={(file) => {
                // console.log({ file });
                return false;
              }}
            >
              <p className="ant-upload-drag-icon mb-0">
                <InboxOutlined />
              </p>
              <p className="ant-upload-text">
                {'Nhấp hoặc kéo tệp vào khu vực này để tải tệp lên '}
              </p>
              <p className="ant-upload-hint">
                {'File tải lên có định dạng là:  WORD, PDF | Tối đa 50MB'}
              </p>
            </Upload.Dragger>
          </Form.Item>
        </div>
        <Button
          htmlType="submit"
          className="py-4 mt-2 px-4 w-full bg-infoColor hover:bg-infoColorHover font-medium"
          size="large"
          type="primary"
          // shape="round"
          icon={<CheckOutlined />}
        >
          Gửi báo cáo sự cố
        </Button>
      </Form>
    </div>
  );
}
