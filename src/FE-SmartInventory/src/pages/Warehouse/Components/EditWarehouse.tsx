...
interface EditWarehouseProps {
  handleUpdateWarehouse: (id: string, data: TCreateWarehouse) => void;
  warehouse: TWarehouse;
}

const EditWarehouse: React.FC<EditWarehouseProps> = ({ handleUpdateWarehouse, warehouse }) => {
  ...
  useEffect(() => {
    if (warehouse) {
      // Xác định xem kho này là kho cha hay kho con
      const isParent = !warehouse.warehouseId;
      setIsParentWarehouse(isParent);
  ...
  const onFinish = (values: any) => {
    ...
    handleUpdateWarehouse(warehouse.id, dataSubmit);
    form.resetFields();
  };
          ...
          <Col span={12}>
            ...
            >
              <Select placeholder="Chọn kho cha">
                {getAllWarehouse.data?.data
                  .filter((item) => item.id !== warehouse.id) // Loại trừ chính kho đang chỉnh sửa
                  .map((item) => (
                    <Select.Option key={item.id} value={item.id}>
                      {item.name}
                    </Select.Option>
