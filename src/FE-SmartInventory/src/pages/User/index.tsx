...
export default function User() {
  ...
  const { getAllUnit } = useQueryUnit(useBuilderQuery(filterUnit), {
  ...
  const unitName = listUser?.data?.[0]?.unit?.name || 'N/A';
        ...
        {role !== RoleEnumString.DEV && (
          <Tabs defaultActiveKey="all">
            ...
            {Object.keys(usersByRole).map((role) => (
              <TabPane
                tab={
                  role === RoleEnumString.OPERATION ? (
                    <div className="text-sm font-semibold uppercase">
                      <FundOutlined className="mr-2" />
                      {roleName[role as RoleEnumString]}
                    </div>
                  ) : role === RoleEnumString.SUPERVISION ? (
                    <div className="text-sm font-semibold uppercase">
                      <EyeOutlined className="mr-2" />
                      {roleName[role as RoleEnumString]}
                    </div>
                  ) : role === RoleEnumString.TECHNICAL ? (
                    <div className="text-sm font-semibold uppercase">
                      <SettingOutlined className="mr-2" />
                      {roleName[role as RoleEnumString]}
                    </div>
                  ) : role === RoleEnumString.MAINTENANCE ? (
                    <div className="text-sm font-semibold uppercase">
