...
import { Breadcrumb, Button, Layout, Menu, notification, Space, Tag } from 'antd';
...
export default function PageContent({}: PageContentProps) {
  ...
  const openNotificationWithIcon = (type: NotificationType, notifyData: TOpenNotification) => {
    console.log(
      notifyData.type !== TNotifyType.TICKET_OWNER,
      notifyData.type !== TNotifyType.SCHEME_UNDO
    );
    const btnViewTicket = (
      <Space>
        ...
        {notifyData.type !== TNotifyType.TICKET_UNDO &&
          notifyData.type !== TNotifyType.SCHEME_UNDO && (
            <Button
              type="primary"
              size="middle"
  ...
  const handleSyncNotifyData = () => {
    connection!.commandNotify = async (res: TOpenNotification) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.GET_ALL_SCHEME],
      });
      queryClient.invalidateQueries({
  ...
  useEffect(() => {
    ...
    userId && unitId && handleGetNotification();
  }, [userId, unitId]);
